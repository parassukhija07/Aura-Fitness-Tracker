...
function SetRow({ s, i, exId, isLast, planned, onChange, onToggle, onType, onNote, onDelete, showNote, toggleNote }) {
    ...
    React.createElement('div', { className: 'set-block' },
      React.createElement('div', { className: 'set-row' + (s.done ? ' done' : '') },
        ...
        React.createElement('div', { className: 'set-input' },
          React.createElement('input', { type: 'tel', value: s.weight, placeholder: '–',
            onChange: e => onChange(i, 'weight', e.target.value),
            onBlur: () => filled && onToggle(i, true) }),
          React.createElement('label', null, 'kg')
        ),
        React.createElement('div', { className: 'set-input' },
          React.createElement('input', { type: 'tel', value: s.reps, placeholder: '–',
        ...
        React.createElement('button', { className: 'set-note-btn', onClick: () => toggleNote(i) },
          React.createElement(Icon, { name: 'note', size: 16 })
        )
      ),
      showNote && React.createElement('div', { className: 'set-note' },
        React.createElement('input', { value: s.note || '', placeholder: 'Add a note for this set…',
          onChange: e => onNote(i, e.target.value), autoFocus: true }),
        planned !== undefined && React.createElement('button', { className: 'set-del', onClick: () => onDelete(i) },
          React.createElement(Icon, { name: 'trash', size: 15 }), ' Delete set')
      )
    )
  );
}
...
function ExerciseView(props) {
  const { ex, exIdx, onBack, onChange, onToggle, onAddSet, onDelete, onComplete,
          onOpenType, onOpenMenu, onSetNote, onPulley } = props;
  ...
  return React.createElement('div', { className: 'phone', 'data-theme': 'light' },
    ...
    React.createElement('div', { className: 'navbar bordered' },
      React.createElement('div', { className: 'navbar-row' },
        React.createElement('button', { className: 'nav-btn', onClick: onBack },
          React.createElement(Icon, { name: 'chevron-left', size: 22 }), 'Workout'),
        React.createElement('div', { className: 'nav-title', style:{fontSize:'15px'} }, `Exercise ${exIdx + 1}`),
    ...
    React.createElement('div', { className: 'screen-body pad pad-b' },
      ...
      React.createElement('h2', { style:{fontSize:'24px',fontWeight:800,letterSpacing:'-.02em',margin:'14px 0 8px'} }, ex.name),
      ...
      React.createElement('div',{className:'sets'},
        ex.sets.map((s,i)=>React.createElement(SetRow,{
      ...
      React.createElement('button',{className:'btn btn-primary',style:{marginTop:'18px'},onClick:onComplete},
